...
void preempt_start(void)
{
	...
	new_timer.it_value.tv_usec = PREEMPT_FREQ; /* 10ms = 10000 microseconds */
	/* ... and every 10ms after that */
	new_timer.it_interval.tv_sec = 0;
	new_timer.it_interval.tv_usec = PREEMPT_FREQ;
			
	/* stores information about the timer specified by which in the structure pointer by old */
...
{
	/* if and when the user's handler returns normally, the original mask is restored - restore previous action */
	if (act_restored == 1){
